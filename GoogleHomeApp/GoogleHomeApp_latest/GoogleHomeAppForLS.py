...
def GoogleHomeApp(i):
    ...
    try:
        ...
        if errorString.lower() == errorList[0].lower():
            writeToFileRunStatus('GoogleHomeSetup', 'GoogleHomeSetup_Run'+str(i)+' '+'='+' '+'what are you setting up')
        if errorString.lower() == errorList[1].lower():
            LSDevice.captureLogs('Could_not_comm_with_your_device_'+'run'+str(i))
            writeToFileRunStatus('GoogleHomeSetup', 'GoogleHomeSetup_Run'+str(i)+' '+'='+' '+'Could not Communicat/Authenticate with your device')
        if errorString.lower() == errorList[2].lower():
            LSDevice.captureLogs('Something_went_wront_'+'run'+str(i))
            writeToFileRunStatus('GoogleHomeSetup', 'GoogleHomeSetup_Run'+str(i)+' '+'='+' '+'Somthing Went Wrong')
        if errorString.lower() == errorList[3].lower():
            LSDevice.captureLogs('Turn_off_ap_isolation_'+'run'+str(i))
            writeToFileRunStatus('GoogleHomeSetup', 'GoogleHomeSetup_Run'+str(i)+' '+'='+' '+'Turn Off Ap Isolation')
        if errorString.lower() == errorList[4].lower():
            LSDevice.captureLogs('Connection_problem_during_setup_'+'run'+str(i))
            writeToFileRunStatus('GoogleHomeSetup', 'GoogleHomeSetup_Run'+str(i)+' '+'='+' '+'Connection_problem_during_setup/PSK/wrong password issue')
        if errorString.lower() == errorList[5].lower():
            LSDevice.captureLogs('SuccessfullWIFI Setup_ '+'run'+str(i))
            writeToFileRunStatus('GoogleHomeSetup', 'GoogleHomeSetup_Run'+str(i)+' '+'='+' '+' Setup PASS')
        if errorString.lower() == errorList[6].lower():
            LSDevice.captureLogs('No Device Found_ '+'run'+str(i))
            writeToFileRunStatus('GoogleHomeSetup', 'GoogleHomeSetup_Run'+str(i)+' '+'='+' '+' No Device Found. Make sure device are turned on and available to connect.')
